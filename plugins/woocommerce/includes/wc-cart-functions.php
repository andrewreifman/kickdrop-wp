...
function wc_add_to_cart_message( $product_id ) {
	...
	if ( get_option( 'woocommerce_cart_redirect_after_add' ) == 'yes' ) :
		...
		$message 	= sprintf('<a href="%s" class="button wc-forward">%s</a> <p>%s</p>', $return_to, __( 'Continue shopping', 'woocommerce' ), $added_text );

	else :

		$message 	= sprintf('<a href="%s" class="button wc-forward">%s</a> <p>%s</p>', get_permalink( wc_get_page_id( 'cart' ) ), __( 'View Cart', 'woocommerce' ), $added_text );
